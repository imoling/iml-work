// 业务系统连接 — 本地登录验证 + 登录保活心跳（凭证只在本地受管浏览器 Profile，绝不上传）。
import { ipcMain } from 'electron'
import { rt, chromium, profileDir, launchCtx } from './runtime'
...
export function register(): void {
  ...
  ipcMain.handle('connection:ping', async (_e, { systemId, baseUrl }: any) => {
    if (rt.recorderCtx || rt.verifyCtx || rt.dryCtx) return { ok: true, skipped: true }   // 有其它浏览器任务占用同一 Profile，本轮跳过
    let ctx: any = null
    try {
