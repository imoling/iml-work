...
// 产物 node_modules/.bench/router-media-probe.mjs（gitignored）；external 包运行时从 node_modules 解析。
...
await build({
  entryPoints: [path.join(BENCH, 'router-media-probe.ts')],
  bundle: true,
  platform: 'node',
  format: 'esm',
  target: 'node20',
  outfile: path.join(CLIENT, 'node_modules/.bench/router-media-probe.mjs'),
...
console.log('bundle OK → node_modules/.bench/router-media-probe.mjs')
