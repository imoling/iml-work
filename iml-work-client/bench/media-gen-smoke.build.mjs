// 打包多媒体生成冒烟：electron → 桩、src/main/db → 桩，其余真实管线模块原样打进 bundle。
// 产物 node_modules/.bench/media-gen-smoke.mjs（gitignored）；external 包运行时从 node_modules 解析。
...
await build({
  entryPoints: [path.join(BENCH, 'media-gen-smoke.ts')],
  bundle: true,
  platform: 'node',
  format: 'esm',
  target: 'node20',
  outfile: path.join(CLIENT, 'node_modules/.bench/media-gen-smoke.mjs'),
...
console.log('bundle OK → node_modules/.bench/media-gen-smoke.mjs')
