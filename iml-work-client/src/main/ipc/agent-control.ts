...
import { ballTaskAborted } from '../float-ball'
...
export function registerAgentControlHandlers() {
  ...
  // 小影立即回位（不等被中止的 promise 落地，也不触发「搞定」庆祝）
  ipcMain.handle('agent:abort', (_e, runId?: string) => { abortRun(runId); ballTaskAborted(runId); return { success: true } })
