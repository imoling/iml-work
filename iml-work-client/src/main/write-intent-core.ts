...
/**
 * 任务级写动词：判断**用户这句任务**是不是要改动业务系统（区别于上面三层——那些判的是「页面按钮」）。
 * 只用来决定走哪个执行引擎，**不再充当安全判据**：判漏了也只是走进通用内核，
 * 那里的 browse 工具照样在点击写按钮前签字。
 */
export const WRITE_TASK_VERB =
  /(提交|审批|批准|同意|通过|驳回|退回|办理|处理|新建|创建|发起|录入|填写|上报|打卡|签到|签退|删除|修改|更新|保存|下单|预约|申请)/
