...
import { WRITE_INTENT_LABEL } from './skill-exec'
import { runNonSystemSkillForm } from './skill-forms'
...
export async function runCustomSkill(matchedSkill: SkillDefinition, skl: string, data: AgentTaskData, sendLog: SendLog, trace: AgentTrace, out: SkillExecOut, focusHint?: string, materials?: string): Promise<AgentResult | null> {
  let skillHandled = false
      ...
      // 「不碰业务系统」的四种形态（深度调研 / 沙箱代码型 / agentic bundle / 知识指南型）
      // 已拆至 skill-forms.ts（体检 P2-16）：命中即在那里跑完，未命中继续走下方业务系统链路。
      if (await runNonSystemSkillForm({ matchedSkill, skl, data, sendLog, trace, out, skillType, skillCode, skillSop, skillBundle, focusHint, materials })) return null
