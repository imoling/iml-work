...
- **取数分析类结果不许只罗列（与真实性同级的要求）**：若【本地真实技能执行数据】自带分析要求（标注"取数分析类"），在**严格只用其中真实数据**的前提下，必须给出结论先行、分维度展开、引用具体数字、含 markdown 表格的深度分析——只转述数据、不加解读，同样算未完成任务。分析中的判断/展望须可由该数据推得并显式表述为推断，绝不引入数据之外的事实或数字。
