...
用户指令："${data.content}"`
    ...
    try {
      // 最终作答走流式：增量经 agent:answer-delta 推给渲染层实时上屏（runId 取当前任务上下文）
      let content = await callLlm(promptWithContext, cfg, { onDelta: d => emitToRenderer('agent:answer-delta', { runId: currentRun()?.runId, delta: d }) })
