...
export const SEMANTIC_FN = `function(step){
  return new Promise(function(resolve){
    ...
    try {
      ...
      if (op==='fill'){ withRetry(function(){ var c=bySel()||labelControl(arg)||attrControl(arg)||geoControl(arg)||floatSearchControl(); if(c&&(c.readOnly||c.disabled)){ var alt=floatSearchControl(); if(alt) c=alt; } if(!c) return false; if(step.locateOnly){ resolve(locOf(c)); return true; } c.focus(); try{ c.click(); }catch(e){} setNativeValue(c,value);
      ...
      if (op==='searchSelect'||op==='search'){ /* search=录制 IR 动词,兼容别名(存量技能可能带) */ withRetry(function(){ var c=bySel()||labelControl(arg)||attrControl(arg)||geoControl(arg)||floatSearchControl(); if(c&&(c.readOnly||c.disabled)){ var alt=floatSearchControl(); if(alt) c=alt; } if(!c) return false; if(step.locateOnly){ resolve(locOf(c)); return true; } c.focus(); try{ c.click(); }catch(e){} setNativeValue(c,value); pollClickOption(value, resolve, c); return true; }); return; }
      if (op==='openPicker'){ /* 放大镜/拾取器控件：点开检索弹窗（值必须经弹窗搜索选中——行内输入框直接打字留不住的场景）。
        打开后由 agent 在弹窗里 fill 搜索框 → 点查询 → click 结果项 →（如有）click 确定。 */
        // 录制锚点优先：step.sel 给了就是那个放大镜/开弹窗按钮，直接定位/点它（最确定，不用几何猜）。
        if(step.sel){ try{ var btn0=document.querySelector(step.sel); if(btn0 && visible(btn0)){ btn0.scrollIntoView({block:'center'}); if(step.locateOnly){ resolve(locOf(btn0)); return; } try{ btn0.click(); }catch(e){} resolve({ok:true}); return; } }catch(_e){} }
        withRetry(function(){
