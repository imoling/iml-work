...
export function fireScheduledTask(t: ScheduledTask, trigger: 'schedule' | 'manual' = 'schedule') {
  schedSetLastRun(t.id, Date.now())
  emitToRenderer('schedule:fire', { id: t.id, title: t.title, prompt: t.prompt, expertId: t.expertId, expertName: t.expertName, trigger })
