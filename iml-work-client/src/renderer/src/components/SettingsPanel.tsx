...
// 远程控制机器人（微信/飞书/钉钉/QQ）已拆至 settings/RobotTab.tsx。
...
export default function SettingsPanel({ initialTab }: SettingsPanelProps) {
  ...
  // 工作空间页 / 远程机器人页已拆至 settings/FolderTab.tsx、settings/RobotTab.tsx。

  const [saving, setSaving] = useState(false)
  const [testResult, setTestResult] = useState<any>(null)
  const [testing, setTesting] = useState(false)

  // 企业系统连接页已拆至 settings/SystemsTab.tsx（状态与处理器随之迁移）。
