...
这条手搓的智能体管线不是黑箱——我们用主流公开测试集量化过它的水平，harness 和题库都在 [`iml-work-client/bench/`](iml-work-client/bench/)，`npm run eval:bench` 一键可复跑。挑几个能打的说：

| 测试集 | 考察 | 我们 | 参照 |
|---|---|---|---|
| **Chinese-SimpleQA** | 中文事实问答 | **90%** | 公开榜最好成绩 o1-preview 63.8%、GPT-4o 59.3% —— **超出一大截** |
| **GSM8K** | 数学推理 | **96.7%** | 与同款模型裸调持平，管线不拖后腿 |
| **IFEval** | 指令/格式遵循 | **90%** | 前沿模型 93~95% 区间 |
| **SimpleQA** | 英文长尾事实（带检索） | **70%** | 无工具前沿 33~75%，进入带检索可用区间 |
...
完整测试报告（含逐题判分、行业对比、与 Manus/Genspark 等同类产品在 GAIA 上的对标、三轮迭代优化）在 `docs/agent-bench-report-*.md`。
