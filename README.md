...
这条手搓的智能体管线不是黑箱——我们用主流公开测试集量化过它的水平，harness 和题库都在 [`iml-work-client/bench/`](iml-work-client/bench/)。同题、同模型（DeepSeek）、同判分口径，看三层递进的价值——裸模型 → 旧链路 → **新内核 TurnEngine**（一个循环+一张工具表的原生 function-calling 执行引擎）：

| 测试集 | 考察 | 裸 DeepSeek | 旧链路 | **新内核 TurnEngine** |
|---|---|---|---|---|
| **SimpleQA** | 英文长尾事实 | 30.0% | 66.7% | **73.3%** |
| **C-SimpleQA** | 中文事实 | 76.7% | 90.0% | **93.3%** |
| **FRAMES** | 多跳检索 | 40.0% | 60.0% | **65.0%** |
| **GAIA** | 文本/工具题 | 25.0% | 25.0% | **70.0%** 🚀 |
| **GSM8K** | 数学推理 | 100.0% | 96.7% | 96.7% |
| **IFEval** | 指令/格式遵循 | 85.0% | 90.0% | 85.0% |
| **平均** | | 59.4% | 71.4% | **80.5%** |

**新内核 TurnEngine 是核心跃升**：学术平均 71.4→**80.5**，除 IFEval 外全面超越旧链路。最亮的是 **GAIA 25→70（+45）**——逼近官方顶级 agent 的 ~75%，逐题核实真对（连视频题都靠 web_search 搜到答案；答不出的诚实说"无法确定"、不乱编，守住红线）。IFEval 略降 5 是唯一短板（旧链路有个确定性格式校验器、TurnEngine 还没接，可补上）。行业坐标：FRAMES 的 65% 已达官方「多步 RAG」66%（Gemini-1.5）——**TurnEngine 用 DeepSeek 达到了行业主流多步检索水平**。代价是慢（function-calling 多轮 + 深读，GAIA 中位 ~200s）：拿时延换"办得对"。

几个数字背后的东西：

- **不编造是有实测支撑的产品主张**。同样的题，接上真实性红线后错答率相对裸模型直接减半（中文 27%→13%、英文 57%→23%）——分身宁可如实说"没查到"，也绝不给你编一个单号金额人名。这正是企业场景敢把它放进审批流的底气。
- **中文事实能力是真的强**。C-SimpleQA **93.3%** 高于所有公开榜成绩，员工问公司制度、行业常识、业务事实，答得住。
- **该联网时联网、该本地算时本地算**。需要外部信息就联网多跳补查、信源分级、日期纪律全程在线；自足的计算题用沙箱精确算不心算。新内核为了追得准，宁可多走几轮、慢一些——用时延换"办得对"。
